...
namespace NewLMS.UMKM.MediatR.Features.LoanApplications.Queries.GetInbox
{
    ...
    public class GetAppInboxQueryHandler : IRequestHandler<GetAppInboxQuery, ServiceResponse<List<AppInboxResponse>>>
    {
        ...
        public async Task<ServiceResponse<List<AppInboxResponse>>> Handle(GetAppInboxQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var dataInbox =
                   (from _loan in _userContext.LoanApplications
                    join _debtor in _userContext.Debtors on _loan.NoIdentity equals _debtor.NoIdentity into joinedDebtor
                    from _debtor in joinedDebtor.DefaultIfEmpty()
                    join _branch in _userContext.RfBranches on _loan.RfBranchId equals _branch.Code into joinedBranch
                    ...
                    {
                        Id = _loan.Id,
                        LoanApplicationId = _loan.LoanApplicationId,
