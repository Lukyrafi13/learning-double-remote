...
{
    public class BackToPrescreenLoanApplicationAnalystCommand : LoanApplicationProcessStageRequest, IRequest<ServiceResponse<Unit>>
    {
    }

    public class BackToPrescreenLoanApplicationAnalystCommandHandler : IRequestHandler<BackToPrescreenLoanApplicationAnalystCommand, ServiceResponse<Unit>>
    {
        ...
        public BackToPrescreenLoanApplicationAnalystCommandHandler(
        ...
        public async Task<ServiceResponse<Unit>> Handle(BackToPrescreenLoanApplicationAnalystCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (loanApplication != null)
                {
                    ...
                    LoanApplicationStage loanApplicationStage = LoanApplicationHelper.CreateLoanApplicationStage(loanApplication, Guid.Parse(_currentUser.Id), UMKMConst.Stages["Prescreening"], loanApplication.CreatedBy, UMKMConst.Roles["AccountOfficerUMKM"]);

                    await _loanStage.AddAsync(loanApplicationStage);
                    #endregion


                    loanApplication.StageId = UMKMConst.Stages["Prescreening"];
