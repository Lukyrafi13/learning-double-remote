    ...
    public class GetApprLandBuildingQueryHandler : IRequestHandler<GetApprLandBuildingQuery, ServiceResponse<ApprWorkPaperLandBuildingHeaderResponse>>
    {
                ...
                foreach (var data in apprSummary.ApprWorkPaperLandBuildings)
                {
                    if (data.DataNumber == 0)
                    {
                        var response = new ApprLandBuildingResponse()
                        {
                            ...
                            LandDocument = _mapper.Map<SimpleResponse<Guid>>(data?.ApprLandTemplates?.CertificateTypeFK),
                            LandForm = _mapper.Map<SimpleResponse<Guid>>(data?.ApprLandTemplates?.LandShapeFK),
                            LandCondition = _mapper.Map<SimpleResponse<Guid>>(data?.ApprLandTemplates.LandTypeFK),
        ...
        private async Task<ApprWorkPaperLandBuildingSummaries> GetWorkPaperSummary(Guid appraisalGuid)
        {
            var include = new string[] {
                    ...
                    "ApprWorkPaperLandBuildings.ApprLandTemplates.CertificateTypeFK",
                    "ApprWorkPaperLandBuildings.ApprLandTemplates.LandShapeFK",
                    "ApprWorkPaperLandBuildings.ApprLandTemplates.LandTypeFK",
