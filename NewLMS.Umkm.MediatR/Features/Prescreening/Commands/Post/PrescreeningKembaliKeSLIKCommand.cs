...
{
    public class PrescreeningKembaliKeSLIKCommand : PrescreeningFind, IRequest<ServiceResponse<PrescreeningProsesResponse>>
    {
        public string NamaUser { get; set; }
    }
    public class PrescreeningKembaliKeSLIKCommandHandler : IRequestHandler<PrescreeningKembaliKeSLIKCommand, ServiceResponse<PrescreeningProsesResponse>>
    {
        ...
        public PrescreeningKembaliKeSLIKCommandHandler(
        ...
        public async Task<ServiceResponse<PrescreeningProsesResponse>> Handle(PrescreeningKembaliKeSLIKCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var SLIK = await _SlikRequest.GetByPredicate(x=> Prescreening.AppId == x.AppId);

                
                if (SLIK == null){
                    var failResp = ServiceResponse<PrescreeningProsesResponse>.Return404("Data SLIK not found");
                    failResp.Success = false;
                    return failResp;
                }
                ...
                if (Prescreening.App.Prospect.Stage.Code == "4.2.1"){
                ...
                // Reset SLIK Stage
                SLIK.AdminVerified = 0;
                SLIK.ProcessStatus = 0;

                await _SlikRequest.UpdateAsync(SLIK);
