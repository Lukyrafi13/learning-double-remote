        ...
        public async Task<ServiceResponse<SlikRequestObjectResponseDto>> Handle(SlikRequestObjectUploadCommandPostCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (request.File != null){
                    ...
                    var FolderName = "slikadmins/" + $"{exisitingSlikRequest.LoanApplication.LoanApplicationId}{"_"}{Regex.Replace(exisitingSlikRequest.LoanApplication.LoanApplicationId, "[^A-Za-z0-9]+", "_")}";
                    ...
                    if(existingSlikRequestObjectUploadCommand.SLIKDocumentUrl != null) // Update
                    {
                        var fileUrl = await _FileUrl.GetByIdAsync((Guid)existingSlikRequestObjectUploadCommand.SLIKDocumentUrl, "Id");
                            fileUrl.Url = await request.File.SaveToLocal(_GetSaveDir(request.File, FolderName, exisitingSlikRequest.LoanApplication.LoanApplicationId.ToString(), "SLIK", timestamp));
                            fileUrl.FileSize = request.File.Length.ToString();
                            fileUrl.FileType = await request.File.getType(_GetSaveDir(request.File, FolderName, exisitingSlikRequest.LoanApplication.LoanApplicationId.ToString(), "SLIK", timestamp));
                    ...
                    else  // Insert
                    {
                        var FileUrl = new FileUrl
                        {
                            ...
                            Url = await request.File.SaveToLocal(_GetSaveDir(request.File, FolderName, exisitingSlikRequest.LoanApplication.LoanApplicationId.ToString(), "SLIK", timestamp)),
                            FileSize = request.File.Length.ToString(),
                            FileType = await request.File.getType(_GetSaveDir(request.File, FolderName, exisitingSlikRequest.LoanApplication.LoanApplicationId.ToString(), "SLIK", timestamp))
                        };
